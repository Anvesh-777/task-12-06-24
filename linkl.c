...
//	memcpy(staInfo->hfid, gSysProfile.systemName, MAX_SYSTEM_NAME);
	...
	memcpy(staInfo->dfltKey, gSysProfile.oemKey, ENC_KEY_LEN);
...
void LINKL_SetStaMode(sLinkLayer *linkLayer)
{
    ...
    HAL_RegisterProcBcnCallback(HOMEPLUG_GetHal(),NULL,
...
void LINKL_SetHpgpNetworkKey(u8 *nwkey)
{
	u8 i;
	u8			pwd[32];
	...
	memcpy(linkl->nwKey, nwkey, ENC_KEY_LEN);

//	FM_HexDump(FM_USER,"linkl nwkey\n", (u8*)nwkey, ENC_KEY_LEN);
	...
	memcpy(staInfo->nek, nwkey, ENC_KEY_LEN);
...
//    LINKL_InitCcoInfo(linkLayer);
  ...
  //  os_create_task(HPGP_TASK_ID_LINK);
	...
	staInfo->dfltKeyEks = 5;
	
#ifdef HPGP_HAL
		/* set the NEK to the LMAC for the CCo */
	HHAL_AddNEK(staInfo->dfltKeyEks, staInfo->dfltKey);
#endif
